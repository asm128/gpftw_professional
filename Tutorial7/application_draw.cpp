...
//
	...
	for(uint32_t iEnemy = 0, enemyCount = gameInstance.Enemies.Alive.size(); iEnemy < enemyCount; ++iEnemy) {
		if(0 == gameInstance.Enemies.Alive[iEnemy])
			continue;
		error_if(errored(::cho::grid_copy_alpha(viewOffscreen, enemyView, gameInstance.Enemies.Position[iEnemy].Cast<int32_t>() - applicationInstance.TextureCenters[GAME_TEXTURE_ENEMY], {0xFF, 0, 0xFF, 0xFF})), "I believe this never fails.");
		{
	...
	for(uint32_t iRay = 0, rayCount = applicationInstance.StuffToDraw.Debris.size(); iRay < rayCount; ++iRay) {
		...
		if(applicationInstance.ParticleSystem.Instances[particleToDraw.IndexParticle].Binding.Lit) {
...
static				const ::cho::array_static<::cho::SColorBGRA, WEAPON_TYPE_COUNT>	weaponTypeColorPalette			= 
	...
	for(uint32_t iRay = 0, rayCount = applicationInstance.StuffToDraw.ProjectilePaths.size(); iRay < rayCount; ++iRay) {
		...
		const ::cho::SParticleBinding<::SGameParticle>								& gameParticle								= applicationInstance.ParticleSystem.Instances[laserToDraw.IndexParticle];
	...
	for(uint32_t iRay = 0, rayCount = applicationInstance.StuffToDraw.Stars.size(); iRay < rayCount; ++iRay) {
		...
		if(false == applicationInstance.ParticleSystem.Instances[starToDraw.IndexParticle].Binding.Lit)
			continue;
	...
	for(uint32_t iThrust = 0, particleCount = (uint32_t)applicationInstance.StuffToDraw.Thrust.size(); iThrust < particleCount; ++iThrust) {
		...
		if(false == applicationInstance.ParticleSystem.Instances[thrustToDraw.IndexParticle].Binding.Lit)
			continue;
